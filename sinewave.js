	...
	while (true) {
		...
		for (const sym of prioritizedSymbols) {
			...
			if (longShares > 0) {
				...
				const holding = 1000000;
				if (state > 0) {
					const sharesToBuy = Math.min(10000, ns.stock.getMaxShares(sym), Math.floor((money - commission - holding) / askPrice));
					if (ns.stock.buy(sym, sharesToBuy) > 0) {
						ns.print(`BOUGHT (long) ${sym}.`);
					}
				} else if (state < 0) {
					const sharesToBuy = Math.min(10000, ns.stock.getMaxShares(sym), Math.floor((money - commission - holding) / bidPrice));
