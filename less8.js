...
export async function main(ns) {
	...
	async function sortServers() {
		...
		for (let i = 0; i <= hTarget.length - 1; i++) {
			let sTarget = hTarget[i]; var serverGrowth = ns.getServerGrowth(sTarget) * -1 + 100;
			if (ns.serverExists(sTarget) && ns.hasRootAccess(sTarget)) {
				if (ns.getServerMaxMoney(sTarget) > 0 || ns.getServerMaxRam(sTarget) > 2) {
					temp = [Math.trunc(ns.getServerMaxMoney(sTarget) * 0.01 / ns.getServerMinSecurityLevel(sTarget) / serverGrowth / ns.getServerRequiredHackingLevel(sTarget)), sTarget]
			...
			if (ns.serverExists(hostList[i][1])) {
				...
				if (Math.trunc(secNum) < 1) {
					if (Math.trunc(mCash / caSh) <= 1) {
						...
						var hThreads = Math.trunc(ramThreads / (hostList.length - 1))
						if (hThreads < 1) { } else {
				...
				if (Math.trunc(secNum) > 0) {
					...
					if (wThreads < 1) { } else {
						if (ns.getWeakenTime(targetList[x][1]) < 300000) {
				...
				if (Math.trunc(mCash / caSh) > 1) {
					...
					if (gThreads < 1) { } else {
						if (ns.getGrowTime(targetList[x][1]) < 300000) {
